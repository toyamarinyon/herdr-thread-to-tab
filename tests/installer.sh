...
test "$asset" = "thread-to-tab-v0.1.0-aarch64-apple-darwin.tar.gz"
asset=$(THREAD_TO_TAB_OS=Linux THREAD_TO_TAB_ARCH=x86_64 "$INSTALLER" --print-asset)
test "$asset" = "thread-to-tab-v0.1.0-x86_64-unknown-linux-gnu.tar.gz"
...
cleanup() {
    rm -f "$fixture/thread-to-tab" "$fixture/LICENSE" "$fixture/thread-to-tab-v0.1.0-x86_64-unknown-linux-gnu.tar.gz" "$fixture/SHA256SUMS" "$fixture/bin/thread-to-tab"
...
tar -czf "$fixture/thread-to-tab-v0.1.0-x86_64-unknown-linux-gnu.tar.gz" -C "$fixture" thread-to-tab LICENSE
printf '%064d  %s\n' 0 thread-to-tab-v0.1.0-x86_64-unknown-linux-gnu.tar.gz >"$fixture/SHA256SUMS"
...
if command -v sha256sum >/dev/null 2>&1; then
    hash=$(sha256sum "$fixture/thread-to-tab-v0.1.0-x86_64-unknown-linux-gnu.tar.gz" | awk '{print $1}')
else
    hash=$(shasum -a 256 "$fixture/thread-to-tab-v0.1.0-x86_64-unknown-linux-gnu.tar.gz" | awk '{print $1}')
fi
printf '%s  %s\n' "$hash" thread-to-tab-v0.1.0-x86_64-unknown-linux-gnu.tar.gz >"$fixture/SHA256SUMS"
